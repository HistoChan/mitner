...
def seed_expansion(relevant_nodes, prob_sup_array, sz, vocab_dict, embedding_mat):
    ...
    for j, relevant_node in enumerate(relevant_nodes):
        ...
        if len(word_class) < sz:
            ...
            word_expanded = sorted(
                range(len(expanded)), key=lambda k: expanded[k], reverse=True
            )
            for i in range(sz):
...
def label_expansion(
    ...
    if manual_num is None:
        while len(all_class_keywords) == len(set(all_class_keywords)):
            ...
            seed_expansion(
                children_nodes, prob_sup_array, current_sz, vocab_dict, embedding_mat
            )
            all_class_keywords = [
                w for relevant_node in children_nodes for w in relevant_node.expanded
            ]
        seed_expansion(
            children_nodes, prob_sup_array, current_sz - 1, vocab_dict, embedding_mat
        )
        # seed_expansion(children_nodes, prob_sup_array, current_sz, vocab_dict, embedding_mat)
    else:
        seed_expansion(
            children_nodes, prob_sup_array, manual_num, vocab_dict, embedding_mat
        )
    if manual_num is None:
    ...
    for relevant_node in relevant_nodes:
        children_nodes = (
            relevant_node.children if relevant_node.children else [relevant_node]
        )
        ...
        if fitting == "mix":
            ...
            expanded_mat = embedding_mat[
                np.asarray(list(set(expanded_class)), dtype="int32")
            ]
            vmf_soft = VonMisesFisherMixture(
                n_clusters=num_children, n_jobs=15, random_state=0
            )
            ...
            for child in children_nodes:
                ...
                vmf_soft = VonMisesFisherMixture(
                    n_clusters=1, n_jobs=15, random_state=0
                )
                vmf_soft.fit(expanded_mat)
...
def bow_pseudodocs(
    ...
    for i in range(n_classes):
        ...
        for j in range(num_doc):
            ...
            docs[i * num_doc + j][:doc_len] = np.random.choice(
                len(prob_vec), size=doc_len, p=prob_vec
            )
